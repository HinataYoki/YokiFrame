...
{
    /// <summary>
    /// 带参数运行时事件的 EasyEvent 容器。
    /// </summary>
    /// <typeparam name="T">参数类型。</typeparam>
    public class EasyEvent<T> : IEasyEvent
    {
        private readonly PooledLinkedList<Action<T>> mEventList = new();
        private List<LinkedListNode<Action<T>>> mPendingRemoveNodes;
                    ...
                    if (current != null)
                    {
                        ...
                        catch (Exception e)
                        {
                            EventKitErrorHandler.OnError?.Invoke(
                                $"[EasyEvent<{typeof(T).Name}>] Class {current.Method?.DeclaringType} Method {current.Method?.Name} Error: {e.Message}\n{e.StackTrace}");
                        }
                    }
                    node = nxt;
                }
        ...
        internal void UnRegisterNode(LinkedListNode<Action<T>> node)
        {
            ...
            if (mTriggerDepth > 0)
            {
                ...
                if (mPendingRemoveNodes == null)
                    mPendingRemoveNodes = new List<LinkedListNode<Action<T>>>();
