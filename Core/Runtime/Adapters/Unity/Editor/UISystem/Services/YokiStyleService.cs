...
namespace YokiFrame.Unity
{
    ...
    public static class YokiStyleService
    {
        ...
        private static string ResolveStyleSheetPath(string path)
        {
            ...
            // 相对于 Unity Adapter 根目录，Kit 专属编辑器样式跟随各自 Kit 实现。
            if (path.StartsWith("Runtime/"))
            {
                return YokiStyleCompatibility.ResolveStyleSheetPath(YokiEditorPaths.CombineWithUnityRoot(path));
            }
