...
namespace YokiFrame.EditorTools
{
    ...
    public partial class DocumentationToolPage : YokiFrameToolPageBase
    {
        ...
        private VisualElement CreateTocPanel()
        {
            ...
            mHighlightIndicator.style.backgroundColor = new StyleColor(new Color(0.25f, 0.25f, 0.28f));
        ...
        private VisualElement CreateTocItem(DocModule module, int index)
        {
            ...
            // 图标容器过渡动画
            iconContainer.style.transitionProperty = new List<StylePropertyName>
            {
                new("scale"),
                new("background-color")
            };
            iconContainer.style.transitionDuration = new List<TimeValue>
            {
                new(150, TimeUnit.Millisecond),
                new(150, TimeUnit.Millisecond)
            };
            ...
            item.RegisterCallback<MouseEnterEvent>(evt =>
            {
                if (item != mSelectedTocItem)
                {
                    ...
                    item.style.scale = new Scale(new Vector3(1.02f, 1.02f, 1f));
                    ...
                    iconCont.style.scale = new Scale(new Vector3(1.1f, 1.1f, 1f));
                }
            });
            item.RegisterCallback<MouseLeaveEvent>(evt =>
            {
                if (item != mSelectedTocItem)
                {
                    ...
                    iconCont.style.scale = new Scale(Vector3.one);
                }
            });
