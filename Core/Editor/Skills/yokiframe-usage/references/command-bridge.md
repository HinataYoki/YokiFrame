...
| `InputKit` | `stats` | 输入后端、设备、动作、缓冲和上下文统计 |
| `InputKit` | `get_workbench_snapshot` | 统计、动作状态和上下文栈 |
| `InputKit` | `list_actions` | 只读动作状态列表 |
| `InputKit` | `list_contexts` | 只读上下文栈、注册上下文和启用 ActionMap |
...
返回 `data.stats`、`data.actions` 和 `data.contexts`。AI 默认优先读 `InputKit/state` snapshot；命令桥只做只读诊断，展示 `IInputBackend`、当前设备、动作状态、输入缓冲数量、启用 ActionMap 和上下文栈。按键注入、输入模拟、重绑定和上下文切换不通过 `.yokiframe` 执行。
