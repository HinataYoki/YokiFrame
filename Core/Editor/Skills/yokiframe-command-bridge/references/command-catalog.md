...
`save_editor_tool_settings` 接收面板创建 payload 中的 `PrefabFolder`、`ScriptFolder`、`ScriptNamespace`、`AssemblyName` 和 `CodeTemplate`，保存为当前 Unity 项目的编辑器参数，供工作台、Inspector 和快捷菜单共用。`save_ui_root_settings` 写入 `Assets/Settings/Resources/UIKitSettings.asset`，保存后在下一次 UIRoot 初始化时生效。Runtime 命令桥不打开、关闭、显示、隐藏、压栈或弹栈面板。Prefab/Bind Editor 工具命令需要用户意图和当前 Unity Selection。
...
然后检查配置的生成代码目录，通常是 `Assets/Scripts/TableKit/`，是否存在 Luban 生成代码和 `TableKit.cs`。运行时找不到表时继续检查数据输出目录、`RuntimePathPattern` 和 ResKit Provider。

GraphKit 是 Tauri 图编辑页面，不是 runtime command handler。不要发送 `GraphKit/stats`、`GraphKit/get_workbench_snapshot`，也不要读取 `GraphKit/state`。排查时看：

| 项目 | 说明 |
|---|---|
| Graph project JSON | 当前 graph、node types、blackboard、placemats、notes、edges、subgraphs。 |
| Luban Definition XML | 导出给 TableKit/Luban 的结构定义。 |
| Luban Data XML | 导出给 TableKit/Luban 的 graph 数据。 |
| GraphRuntime contract JSON | 项目侧 graph runtime / handler 对接契约。 |

若 GraphKit 已导出到 TableKit，再进入 TableKit 页面验证和生成；不要绕到 `.yokiframe` runtime 命令。
