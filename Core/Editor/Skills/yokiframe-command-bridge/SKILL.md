...
description: 通过 YokiFrame 文件命令桥查询和调试框架 Kit 状态、snapshot、命令响应、文件桥健康、Unity/Godot engine registry、TableKit/Luban 环境，以及 System、Architecture、FsmKit、EventKit、PoolKit、LogKit、ResKit、SingletonKit、ActionKit、AudioKit、SaveKit、LocalizationKit、SceneKit、SpatialKit、InputKit、UIKit 的命令桥入口。
---
...
TableKit 是编辑器/Luban 工具流，不是 Runtime command handler；AI 只读 `engine.json` 的 optional dependency 和项目生成目录，不发送 `TableKit/*` 命令，不读取 `TableKit/state`。
...
若可用，再检查项目配置代码输出目录，通常是 `Assets/Scripts/TableKit/`，是否存在 Luban 生成代码和 `TableKit.cs`。
