...
| 页面 | 使用场景 |
| --- | --- |
| <img src="Documentation~/images/yokiframe-workbench-console.png" alt="YokiFrame Kit 调试工作台" width="420"> | **工作台首页**：确认 Unity / Godot 宿主是否已连接，查看 heartbeat、命令队列、FileBridge 状态、运行日志，并安装或同步面向 Codex、Claude Code、Cursor、Windsurf、GitHub Copilot 等 Agent 的 YokiFrame Skill。 |
| <img src="Documentation~/images/yokiframe-workbench-architecture.png" alt="Architecture 注册表" width="420"> | **Architecture**：查看当前架构实例、注册服务数量、初始化状态和服务实现列表，用于确认游戏入口、系统服务和依赖注册是否处于 Ready 状态。 |
| <img src="Documentation~/images/yokiframe-workbench-fsmkit.png" alt="FsmKit 状态机工作台" width="420"> | **FsmKit**：查看活跃状态机、当前状态、状态流图和转换历史，适合排查状态跳转、循环风险和运行时流程是否符合预期。 |
| <img src="Documentation~/images/yokiframe-workbench-poolkit.png" alt="PoolKit 对象池监视" width="420"> | **PoolKit**：监视对象池容量、借出对象、峰值和泄漏风险，必要时切换跟踪、定位借出对象或清理历史诊断记录。 |
| <img src="Documentation~/images/yokiframe-workbench-reskit.png" alt="ResKit 资源工作台" width="420"> | **ResKit**：查看已加载资源、引用计数、资源类型、加载来源和卸载历史，用于定位资源未释放、重复加载或 Provider 接入问题。 |
| <img src="Documentation~/images/yokiframe-workbench-uikit.png" alt="UIKit 面板与栈" width="420"> | **UIKit**：查看当前面板、面板栈、层级、UIRoot 设置和缓存状态，适合排查面板未打开、层级错误、栈状态异常和 Canvas 配置问题。 |
| <img src="Documentation~/images/yokiframe-workbench-singletonkit.png" alt="SingletonKit 单例工作台" width="420"> | **SingletonKit**：查看 Core / Unity / Godot 单例实例和生命周期状态，确认 MonoSingleton、GodotSingleton 或纯 C# 单例是否正常注册和存活。 |
