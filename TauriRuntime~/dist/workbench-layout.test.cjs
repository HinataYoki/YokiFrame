...
test('UIKit page exposes Unity editor Prefab creation and binding commands through Tauri', () => {
    const js = readFrontendScripts();
    const css = readDistFile('style.css');
    const uikitPage = readDistFile('pages/uikit.js');
    const editorToolsSource = readDistFile('pages/uikit-editor-tools.js');
    ...
    assert.match(editorHandler, /"get_ui_root_settings"/);
    assert.match(editorHandler, /"save_ui_root_settings"/);
    assert.match(editorHandler, /"reset_ui_root_settings"/);
    ...
    assert.match(uikitPage, /const UIKIT_EDITOR_FORM_STORAGE_KEY\s*=\s*'yokiframe\.uikit\.editorForm\.v1'/);
    assert.match(uikitPage, /function syncUIKitProjectStorageScope\(/);
    assert.match(uikitPage, /readProjectScopedStorageItem\(UIKIT_EDITOR_FORM_STORAGE_KEY\)/);
    assert.match(uikitPage, /writeProjectScopedStorageItem\(UIKIT_EDITOR_FORM_STORAGE_KEY,\s*JSON\.stringify\(uikitState\.editorForm\)\)/);
    assert.match(uikitPage, /syncUIKitProjectStorageScope\(\{\s*force:\s*true\s*\}\)/);
    assert.match(appStateSource, /syncUIKitProjectStorageScope\(\)/);
    assert.match(appStatusSource, /scopedStorageChanged && activePage === 'uikit'[\s\S]*loadUIKitWorkbench\(\)/);
    ...
    assert.match(js, /renderUIKitRootSettingsSection\(stats\?\.rootSettings/);
    assert.match(js, /function renderUIKitRuntimeSummarySection\(/);
    assert.match(js, /<div class="uikit-setup-grid" data-uikit-setup-grid>\s*\$\{editorTools\}\s*\$\{renderUIKitRootSettingsSection\(stats\?\.rootSettings \?\? \{\}\)\}\s*<\/div>\s*\$\{renderUIKitRuntimeSummarySection\(stats,\s*stacks\)\}/);
    assert.doesNotMatch(js, /renderUIKitRootSettingsSection\(stats\?\.rootSettings \?\? \{\}\)\}\s*\$\{renderUIKitRuntimeSummarySection\(stats,\s*stacks\)\}\s*\$\{editorTools\}/);
    assert.match(js, /uikit-inspector-layout/);
    assert.match(js, /kit-workbench-grid--uikit/);
    assert.match(js, /data-uikit-root-settings/);
    assert.match(js, /UIRoot 设置/);
    assert.match(js, /function renderUIKitRootSettingsSection\(/);
    assert.match(js, /uikit-root-settings--primary/);
    assert.match(js, /uikit-root-settings__metrics/);
    assert.match(js, /renderUIKitRootMetric\('渲染模式'/);
    assert.match(js, /renderUIKitRootMetric\('缩放模式'/);
    assert.match(js, /renderUIKitRootMetric\('排序顺序'/);
    assert.match(js, /renderUIKitRootMetric\('拦截类型'/);
    assert.match(js, /data-uikit-resolution-preset/);
    assert.match(js, /Canvas 配置/);
    assert.match(js, /CanvasScaler 配置/);
    assert.match(js, /GraphicRaycaster 配置/);
    assert.match(js, /renderUIKitRootSelectField\('RenderMode'/);
    assert.match(js, /renderUIKitRootSelectField\('ScaleMode'/);
    assert.match(js, /renderUIKitRootNumberField\('ReferenceResolutionX'/);
    assert.match(js, /renderUIKitRootRangeField\('MatchWidthOrHeight'/);
    assert.match(js, /renderUIKitRootSelectField\('PhysicalUnit'/);
    assert.match(js, /renderUIKitRootNumberField\('FallbackScreenDPI'/);
    assert.match(js, /renderUIKitRootNumberField\('DefaultSpriteDPI'/);
    assert.match(js, /renderUIKitRootNumberField\('DynamicPixelsPerUnit'/);
    assert.match(js, /renderUIKitRootToggleField\('IgnoreReversedGraphics'/);
    assert.match(editorToolsSource, /return renderKitToggle\(label,\s*checked,\s*`data-uikit-root-setting/);
    assert.match(editorToolsSource, /renderKitToggle\('覆盖 Prefab',\s*form\.overwrite,\s*'data-uikit-editor-field="overwrite"'/);
    assert.doesNotMatch(editorToolsSource, /uikit-editor-field--toggle/);
    assert.match(js, /renderUIKitRootSelectField\('BlockingObjects'/);
    assert.match(js, /renderUIKitRootNumberField\('BlockingMask'/);
    assert.doesNotMatch(js, /<details class="uikit-root-settings__advanced"/);
    assert.match(js, /data-uikit-save-root-settings/);
    assert.match(js, /data-uikit-reset-root-settings/);
    assert.match(js, /sendKitCommandData\('UIKit',\s*'get_ui_root_settings'/);
    assert.match(js, /sendKitCommandData\('UIKit',\s*'save_ui_root_settings'/);
    assert.match(js, /sendKitCommandData\('UIKit',\s*'reset_ui_root_settings'/);
    assert.match(js, /ReferenceResolutionX:\s*Number/);
    assert.match(js, /<section class="kit-panel uikit-editor-tools" data-uikit-editor-tools>/);
    assert.doesNotMatch(js, /<details class="uikit-editor-tools"/);
    assert.match(js, /UI 面板创建/);
    assert.match(js, /创建 UIPrefab/);
    assert.match(js, /为选中 Prefab 生成代码/);
    ...
    assert.match(editorToolsSource, /saveUIKitEditorForm\(\)/);
    ...
    assert.match(css, /\.uikit-setup-grid\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+minmax\(0,\s*1fr\)/);
    assert.match(css, /@media \(max-width:\s*1180px\)\s*\{[\s\S]*\.uikit-setup-grid[\s\S]*grid-template-columns:\s*1fr/);
    assert.match(css, /\.kit-workbench-grid--uikit\s*\{[\s\S]*grid-template-columns/);
    assert.match(css, /\.uikit-inspector-layout/);
    assert.match(css, /\.uikit-runtime-strip/);
    assert.match(css, /\.uikit-editor-field \.cmd-select/);
    assert.match(css, /\.uikit-root-settings--primary/);
    assert.match(css, /\.uikit-root-settings__metrics\s*\{[\s\S]*grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\)/);
    assert.match(css, /\.uikit-root-settings__metric/);
    assert.match(css, /\.uikit-root-settings__field--range/);
    assert.match(css, /\.uikit-editor-tools__grid\s+\.kit-toggle,\s*\n\.uikit-root-settings__grid\s+\.kit-toggle\s*\{[\s\S]*width:\s*100%/);
    assert.match(css, /\.uikit-editor-tools__grid\s+\.kit-toggle,\s*\n\.uikit-root-settings__grid\s+\.kit-toggle\s*\{[\s\S]*min-height:\s*32px/);
    assert.match(runtimeHandler, /BuildRootSettingsJson/);
    assert.match(runtimeHandler, /rootSettings/);
    assert.match(runtimeHandler, /physicalUnit/);
    assert.match(runtimeHandler, /fallbackScreenDPI/);
    assert.match(runtimeHandler, /defaultSpriteDPI/);
    assert.match(runtimeHandler, /dynamicPixelsPerUnit/);
    assert.match(runtimeHandler, /ignoreReversedGraphics/);
    assert.match(runtimeHandler, /blockingMask/);
