...
// 如果未调用 InitAsync，首次 TableKit.Tables 访问自动同步加载",
                        Explanation = "开启异步加载模式后，生成代码包含 InitAsync/ReloadAsync 方法；YOKIFRAME_UNITASK_SUPPORT 存在时使用 UniTask，否则使用 System.Threading.Tasks.Task。未开启异步加载模式时不会生成异步入口。可通过 SetAsyncBinaryLoader/SetAsyncJsonLoader 自定义异步加载方式，通过 SetTableFileNames 覆盖预加载的文件列表。配置表 ID 常量可以通过 Luban 的枚举或常量表生成。"
                    }
                }
            };
        }
