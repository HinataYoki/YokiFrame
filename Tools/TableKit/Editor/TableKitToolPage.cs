...
namespace YokiFrame.Editor
{
    /// <summary>
    /// TableKit 工具页面
    /// 提供配置表系统的编辑器界面，支持 Luban 代码和数据生成
    /// </summary>
    public class TableKitToolPage : YokiFrameToolPageBase
    {
        ...
        // Luban 生成配置
        private string mLubanWorkDir;           // Luban 工作目录 (包含 luban.conf)
        private string mLubanDllPath;           // Luban.dll 路径
        private string mTarget;                 // -t 参数: client/server/all
        private string mCodeTarget;             // -c 参数: cs-bin/cs-simple-json 等
        private string mDataTarget;             // -d 参数: bin/json 等
        private string mOutputDataDir;          // 数据输出目录
        private string mOutputCodeDir;          // 代码输出目录
        private bool mUseAssemblyDefinition;    // 是否使用独立程序集
        private string mAssemblyName;           // 独立程序集名称
        private bool mGenerateExternalTypeUtil; // 是否生成 ExternalTypeUtil
        ...
        private TreeView mDataTreeView;
        ...
        private void BuildActionsSection(VisualElement container)
        {
            ...
            // 应用配置按钮
            var applyBtn = new Button(ApplyConfig) { text = "应用配置" };
        ...
        private void GenerateLuban()
        {
            ExecuteLuban(false);
        }

        private void ValidateLuban()
        {
            ExecuteLuban(true);
        }
        ...
        {
            if (node.IsNumber)
                return new StyleColor(new Color(0.7f, 0.9f, 0.7f)); // 绿色 - 数字
            if (node.IsBoolean)
                return new StyleColor(new Color(0.9f, 0.7f, 0.7f)); // 红色 - 布尔
            if (node.IsNull)
                return new StyleColor(new Color(0.5f, 0.5f, 0.5f)); // 灰色 - null
            return new StyleColor(new Color(0.9f, 0.8f, 0.6f)); // 黄色 - 字符串
        }

        /// <summary>
        /// 确保必要的文件存在（Luban 生成可能会覆盖这些文件）
        /// </summary>
        private void EnsureRequiredFiles(StringBuilder logBuilder)
        {
            ...
            // 生成 TableKit 运行时代码（放在根目录）
            logBuilder.AppendLine("正在生成 TableKit 运行时代码...");
            TableKitCodeGenerator.Generate(
                codeDir,
                mUseAssemblyDefinition,
                mGenerateExternalTypeUtil,
                mAssemblyName,
                "cfg"
            );
        ...
        {
            // 配置已保存到 EditorPrefs，无需额外操作
            EditorUtility.DisplayDialog("TableKit", "配置已保存", "确定");
            RefreshStatus();
        }
        ...
        private void RefreshEditorCache()
        {
            ...
            try
            {
                // 通过反射调用生成的 TableKit.TablesEditor
