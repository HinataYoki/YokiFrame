    ...
    internal static partial class UIKitPanelCodeGenerator
    {
            ...
            Dictionary<string, string> relocations = null)
        {
            foreach (UIKitBindNode node in nodes)
            {
                ...
                UIElement owner = node.Bind.GetComponent<UIElement>();
                if (owner != default)
                {
                    string actual = UIKitGeneratedOwnerCodeService.GetScriptPath(owner);
                    bool moving = relocations != null && relocations.TryGetValue(actual, out string target) && target == expected;
                    if (!moving && (actual != expected || owner.GetType().FullName != fullName
                        ...
                        throw new InvalidOperationException("绑定已持有另一脚本身份: " + actual
                            + "；普通生成不会迁移类型，请使用显式转换或目录迁移。");
                }
                string legacy = kind == UIKitBindOutputKind.Component
                    ? layout.ScriptFolder + "/UIComponent/" + node.TypeName + ".cs"
                    : layout.ScriptFolder + "/" + (layout.ElementComponentName.Length == 0 ? layout.PanelName
                        : "UIComponent/" + layout.ElementComponentName) + "/UIElement/" + node.TypeName + ".cs";
            ...
            Dictionary<string, string> relocations = null)
        {
            ...
            foreach (MonoScript script in MonoImporter.GetAllRuntimeMonoScripts())
            {
                ...
                if (!moving && (actual != expected || type.Assembly.GetName().Name != assembly))
